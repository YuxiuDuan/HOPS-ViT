...
def generate_data_mulitTarget(args, pred=None, attnmap_pred=None):
    ...
    if args.softlabel:
        ...
        for i in range(len(pred)):
            label = torch.rand(1000)  # 初始时所有类别的概率为0-1随机
    ...
    for lr_it in range(2):
        ...
        for itr in range(iterations_per_layer):
            ...
            loss_align = (torch.mean(loss_hard + loss_kl)) / 1
            ...
            total_loss = loss_oh + loss_tv + loss_soft + loss_attnmaps
            if itr % 20 == 0:
                logging.info(
