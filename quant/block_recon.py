...
def block_reconstruction(model, block, teacher_model, T_module, cali_data: torch.Tensor,
                         batch_size: int = 32, iters: int = 100, weight: float = 0.01, opt_mode: str = 'mse',
                         ...
                         last_stage: bool = False, regularize: bool = False):
    """
    ...
    optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, block.parameters()), lr=4e-5, betas=(0.9, 0.999),weight_decay=0)
