...
class UniformQuantizer(nn.Module):
    ...
    def custom_clamp(self, inp, min_val, max_val):
        clamped = torch.clamp(inp, min_val, max_val)
        return inp + (clamped - inp).detach()
...
class UniformQuantizer_Attn(nn.Module):
    """
    PyTorch Function that can be used for asymmetric quantization (also called uniform affine
    quantization). Quantizes its argument in the forward pass, passes the gradient 'straight
    through' on the backward pass, ignoring the quantization that occurred.
    ...
    def __init__(self, n_bits: int = 8, channel_wise: bool = False, is_act: bool = False):
        super(UniformQuantizer_Attn, self).__init__()
        assert 2 <= n_bits <= 8, 'bitwidth not supported'
        self.n_bits = n_bits
        self.n_levels = 2 ** self.n_bits
        self.delta = None
    ...
    def forward(self, x: torch.Tensor):
        # x ~ [0, 1]
        x_int = round_ste(x * (self.n_levels-1))
        x_dequant = x_int / (self.n_levels-1)

        return x_dequant

class SULQQuantizer(nn.Module):
    """
    PyTorch Function that can be used for asymmetric quantization (also called uniform affine
    quantization). Quantizes its argument in the forward pass, passes the gradient 'straight
    through' on the backward pass, ignoring the quantization that occurred.
    ...
    """
    def __init__(self, n_bits: int = 8, channel_wise: bool = False, is_act: bool = False):
        super(SULQQuantizer, self).__init__()
        assert 2 <= n_bits <= 8, 'bitwidth not supported'
        self.n_bits = n_bits
        self.n_levels = 2 ** self.n_bits
        self.delta = None
